...
from architecture.response import ResponseQuery, ResponseRecordType, Response
...
class TestResponse(unittest.TestCase):
    ...
    def test_response_init(self):
        response = Response(self.raw_message)
        self.assertEqual(response.header, ResponseHeader(self.raw_message))
        self.assertEqual(len(response.questions), 1)
        self.assertEqual(len(response.answers), 5)
        self.assertEqual(response.authority_RRs, [])
        self.assertEqual(response.additional_RRs, [])
